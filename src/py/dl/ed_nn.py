...
#   output: tensor of computed logits
    ...
    conv1_0 = convolution2d(images, name="conv1_0_op", filter_shape=[5, 5, num_channels, 16], strides=[1,1,1,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    conv1_1 = convolution2d(conv1_0, name="conv1_1_op", filter_shape=[5, 5, 16, 16], strides=[1,1,1,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    pool1_0 = max_pool(conv1_1, name="pool1_0_op", kernel=[1,5,5,1], strides=[1,2,2,1], ps_device=ps_device, w_device=w_device)

    conv2_0 = convolution2d(pool1_0, name="conv2_0_op", filter_shape=[5, 5, 16, 32], strides=[1,1,1,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    conv2_1 = convolution2d(conv2_0, name="conv2_1_op", filter_shape=[5, 5, 32, 32], strides=[1,1,1,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    pool2_0 = max_pool(conv2_1, name="pool2_0_op", kernel=[1,5,5,1], strides=[1,2,2,1], ps_device=ps_device, w_device=w_device)

    conv3_0 = convolution2d(pool2_0, name="conv3_0_op", filter_shape=[5, 5, 32, 64], strides=[1,1,1,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    conv3_1 = convolution2d(conv3_0, name="conv3_1_op", filter_shape=[5, 5, 64, 64], strides=[1,1,1,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    ...
    conv4_0 = convolution2d(pool3_0, name="conv4_0_op", filter_shape=[5, 5, 64, 128], strides=[1,1,1,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    conv4_1 = convolution2d(conv4_0, name="conv4_1_op", filter_shape=[5, 5, 128, 128], strides=[1,1,1,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    up_out_shape_4 = conv3_1.get_shape().as_list()
    up_out_shape_4[0] = batch_size
    up_conv4_0 = up_convolution2d(conv4_1, name="up_conv4_0_op", filter_shape=[5, 5, 64, 128], output_shape=up_out_shape_4, strides=[1,2,2,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    
    # concat5_0 = tf.concat([up_conv4_0, conv3_1], -1)

    conv5_0 = convolution2d(up_conv4_0, name="conv5_0_op", filter_shape=[5, 5, 64, 64], strides=[1,1,1,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    conv5_1 = convolution2d(conv5_0, name="conv5_1_op", filter_shape=[5, 5, 64, 64], strides=[1,1,1,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    up_out_shape_5 = conv2_1.get_shape().as_list()
    up_out_shape_5[0] = batch_size
    up_conv5_0 = up_convolution2d(conv5_1, name="up_conv5_0_op", filter_shape=[5, 5, 32, 64], output_shape=up_out_shape_5, strides=[1,2,2,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    
    # concat6_0 = tf.concat([up_conv5_0, conv2_1], -1)

    conv6_0 = convolution2d(up_conv5_0, name="conv6_0_op", filter_shape=[5, 5, 32, 32], strides=[1,1,1,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    conv6_1 = convolution2d(conv6_0, name="conv6_1_op", filter_shape=[5, 5, 32, 32], strides=[1,1,1,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    up_out_shape_6 = conv1_1.get_shape().as_list()
    up_out_shape_6[0] = batch_size
    up_conv6_0 = up_convolution2d(conv6_1, name="up_conv6_0_op", filter_shape=[5, 5, 16, 32], output_shape=up_out_shape_6, strides=[1,2,2,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    
    # concat7_0 = tf.concat([up_conv6_0, conv1_1], -1)

    conv7_0 = convolution2d(up_conv6_0, name="conv7_0_op", filter_shape=[5, 5, 16, 16], strides=[1,1,1,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)
    conv7_1 = convolution2d(conv7_0, name="conv7_1_op", filter_shape=[5, 5, 16, 16], strides=[1,1,1,1], padding="SAME", activation=tf.nn.relu, ps_device=ps_device, w_device=w_device)

    final = convolution2d(conv7_1, name="final", filter_shape=[1, 1, 16, 1], strides=[1,1,1,1], padding="SAME", activation=None, ps_device=ps_device, w_device=w_device)
