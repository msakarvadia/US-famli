...
class NN(base_nn.BaseNN):
    ...
    #   output: tensor of computed logits
        ...
        pool2_0 = self.max_pool3d(conv2_1, name="pool2_0_op", kernel=[1,3,3,3,1], strides=[1,2,2,2,1], ps_device=ps_device, w_device=w_device)

        conv3_0 = self.convolution3d(pool2_0, name="conv3_0_op", filter_shape=[3,3,3,32,64], strides=[1,1,1,1,1], padding="SAME", activation=tf.nn.leaky_relu, ps_device=ps_device, w_device=w_device)
        conv3_1 = self.convolution3d(conv3_0, name="conv3_1_op", filter_shape=[3,3,3,64,64], strides=[1,1,1,1,1], padding="SAME", activation=tf.nn.leaky_relu, ps_device=ps_device, w_device=w_device)
        pool3_0 = self.max_pool3d(conv3_1, name="pool3_0_op", kernel=[1,3,3,3,1], strides=[1,2,2,2,1], ps_device=ps_device, w_device=w_device)

        conv4_0 = self.convolution3d(pool3_0, name="conv4_0_op", filter_shape=[3,3,3,64,128], strides=[1,1,1,1,1], padding="SAME", activation=tf.nn.leaky_relu, ps_device=ps_device, w_device=w_device)
        conv4_1 = self.convolution3d(conv4_0, name="conv4_1_op", filter_shape=[3,3,3,128,128], strides=[1,1,1,1,1], padding="SAME", activation=tf.nn.leaky_relu, ps_device=ps_device, w_device=w_device)

        up_out_shape_4 = conv3_1.get_shape().as_list()
        up_out_shape_4[0] = batch_size
        up_conv4_0 = self.up_convolution3d(conv4_1, name="up_conv4_0_op", filter_shape=[3,3,3,64,128], output_shape=up_out_shape_4, strides=[1,2,2,2,1], padding="SAME", activation=tf.nn.leaky_relu, ps_device=ps_device, w_device=w_device)
        up_conv4_0 = self.max_pool3d(up_conv4_0, name="up_pool4_0_op", kernel=[1,3,3,3,1], strides=[1,1,1,1,1], ps_device=ps_device, w_device=w_device)
        ...
        up_conv5_0 = self.up_convolution3d(conv5_1, name="up_conv5_0_op", filter_shape=[3,3,3,32,64], output_shape=up_out_shape_5, strides=[1,2,2,2,1], padding="SAME", activation=tf.nn.leaky_relu, ps_device=ps_device, w_device=w_device)
        up_conv5_0 = self.max_pool3d(up_conv5_0, name="up_pool5_0_op", kernel=[1,3,3,3,1], strides=[1,1,1,1,1], ps_device=ps_device, w_device=w_device)
        ...
        up_conv6_0 = self.up_convolution3d(conv6_1, name="up_conv6_0_op", filter_shape=[3,3,3,16,32], output_shape=up_out_shape_6, strides=[1,2,2,2,1], padding="SAME", activation=tf.nn.leaky_relu, ps_device=ps_device, w_device=w_device)
        up_conv6_0 = self.max_pool3d(up_conv6_0, name="up_pool6_0_op", kernel=[1,3,3,3,1], strides=[1,1,1,1,1], ps_device=ps_device, w_device=w_device)
    ...
    def loss(self, logits, data_tuple, class_weights=None):
        ...
        with tf.variable_scope("layer1", reuse=True):
            labels_conv = self.convolution3d(labels, name="conv1_0_op", filter_shape=[3,3,3,self.num_channels,16], strides=[1,1,1,1,1], padding="SAME", activation=tf.nn.leaky_relu, ps_device="/cpu:0", w_device="/gpu:0")
        ...
        with tf.variable_scope("layer1", reuse=True):
            logits_conv = self.convolution3d(logits, name="conv1_0_op", filter_shape=[3,3,3,self.num_channels,16], strides=[1,1,1,1,1], padding="SAME", activation=tf.nn.leaky_relu, ps_device="/cpu:0", w_device="/gpu:0")
        ...
        return (3.0*tf.losses.absolute_difference(labels=labels_conv_flat, predictions=logits_conv_flat) + tf.losses.absolute_difference(labels=labels_flat, predictions=logits_flat))/4.0
