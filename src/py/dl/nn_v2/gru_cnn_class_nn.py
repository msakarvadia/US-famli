...
class N(tf.keras.Model):
    ...
    def make_conv_net(self):
        x0 = tf.keras.Input(shape=[256, 256, self.num_channels])

        x = layers.Conv2D(64, (7,7), strides=(2, 2), padding='same', activation=None, use_bias=False)(x0)
        x = layers.BatchNormalization()(x)
        x = layers.ReLU()(x)
        x = layers.MaxPool2D()(x)
        x = layers.Dropout(self.drop_prob)(x)

        x = layers.Conv2D(128, (3,3), strides=(2, 2), use_bias=False, padding='same')(x)
        x = layers.BatchNormalization()(x)
        x = layers.ReLU()(x)
        x = layers.MaxPool2D()(x)
        x = layers.Dropout(self.drop_prob)(x)

        x = layers.Conv2D(256, (3,3), strides=(2, 2), use_bias=False, padding='same')(x)
        x = layers.BatchNormalization()(x)
        x = layers.ReLU()(x)
        x = layers.MaxPool2D()(x)
        x = layers.Dropout(self.drop_prob)(x)

        x = layers.Conv2D(512, (3,3), strides=(2, 2), use_bias=False, padding='same')(x)
        x = layers.BatchNormalization()(x)
        x = layers.ReLU()(x)
        x = layers.MaxPool2D()(x)
        x = layers.Dropout(self.drop_prob)(x)

        #x = layers.Conv2D(1024, (3,3), strides=(2, 2), use_bias=False, padding='same')(x)
        #x = layers.BatchNormalization()(x)
        #x = layers.ReLU()(x)
        #x = layers.MaxPool2D()(x)
        #x = layers.Dropout(self.drop_prob)(x)

        #TODO:make convnet deeper and remove 2d pooling 
    ...
    def make_gru_network(self):
        ...
        #We could replace conv layers with ResNET architectures - instantiate keras ResNET achirtecture (without the weights)
        conv_net = self.make_conv_net()
        ...
        x_h = layers.Bidirectional(layers.GRU(units=512, activation='tanh', use_bias=False, kernel_initializer="glorot_normal", dropout=self.drop_prob))(x_e)

        x_e = layers.Dropout(self.drop_prob)(x_e)
        x_h = layers.Dropout(self.drop_prob)(x_h)
        #x_h_bwd = layers.Dropout(self.drop_prob)(x_h)
    ...
    def summary(self, train_tuple, tr_step, step):
        ...
        #train_tuple[0] = tf.reshape(train_tuple[0], [train_tuple.shape[0], train_tuple.shape[1], train_tuple.shape[2]*images.shape[3], images.shape[4]])
        #tf.summary.image('features', tf.reshape(train_tuple[0], tf.shape(train_tuple[0]).numpy().tolist() + [1])/255, step=step)
       
        #tf.summary.image('features', train_tuple[0][:,8,:,:,0:4]/255, step=step)
        tf.summary.image('features', train_tuple[0][:,8,:,:,0:3]/255, step=step)
